...
def test_emptyHeadBlock():
    ...
    assert head.encode() == bytes(b'\x41\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x41')

def test_avail_1byte_new():
    head = blockDescriptor.HeadBlock.initAvailable(1, 0, 0, 66)
    assert head.encode() == bytes(b'\x41\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x42')

def test_avail_1byte_linked():
    head = blockDescriptor.HeadBlock.initAvailable(1, 170, 255, 491)
    assert head.encode() == bytes(b'\x41\x00\x00\x00\x01\x00\x00\x00\xaa\x00\x00\x00\xff\x01\xeb')
...
def test_meta_496byte():
    ...
    assert head.encode() == bytes(b'\x4d\x00\x00\x01\xf0\x00\x00\x01\x90\x00\x00\x00\x60\x02\x2f')
...
def test_data_1byte():
    ...
    assert head.encode() == bytes(b'\x44\x00\x00\x00\x01\x00\x00\x00\x01\x00\x00\x00\x00\x00\x46')

def test_data_65_000bytes():
    head = blockDescriptor.HeadBlock.initData(65000, 65000, 0, 1038)
    assert head.encode() == bytes(b'\x44\x00\x00\xfd\xe8\x00\x00\xfd\xe8\x00\x00\x00\x00\x04\x0e')    
...
def test_data_65_000byte_padd():
    ...
    assert head.encode() == bytes(b'\x44\x00\x00\xfd\xe8\x00\x00\x00\x00\x00\x00\xfd\xe8\x04\x0e')    
...
def test_data_65_000both():
    ...
    assert head.encode() == bytes(b'\x44\x00\x00\xfd\xe8\x00\x00\xfd\xe8\x00\x00\xfd\xe8\x05\xf3')    
...
def test_meta_1byte_end():
    ...
    assert tail.encode() == bytes(b'\x00\x00\x00\x01\x4d')
...
def test_meta_496byte_end():
    ...
    assert tail.encode() == bytes(b'\x00\x00\x01\xf0\x4d')
...
def test_data_1byte_end():
    ...
    assert tail.encode() == bytes(b'\x00\x00\x00\x01\x44')
...
def test_data_496byte_end():
    ...
    assert tail.encode() == bytes(b'\x00\x00\x01\xf0\x44')
